...
class AssetServer
{
	...
	{
		$this->contentDAO = new GenericDAO('Content');
		$this->contentTypeDAO = new GenericDAO('ContentType');
	}
	...
	public function replaceAsset($assetId, $data)
	{
		...
		file_put_contents($contentType->getPath() . $content->getPath() . '/' . $assetId, $data);
	}
